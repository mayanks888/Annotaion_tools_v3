...
class LabelTool():
    def __init__(self, master):
        ...
        self.mainPanel = Canvas(self.frame, cursor='tcross', height=2000, width=2000)
        ################################################################
        sbarV = Scrollbar(orient=VERTICAL)
        sbarH = Scrollbar(orient=HORIZONTAL)
        ...
        self.mainPanel.config(scrollregion=(0, 0, 2080, 1700))
        # self.mainPanel.config(scrollregion=(0, 0, 100, 100))
        ########################################################################3
        # self.mainPanel = Canvas(self.frame, cursor='tcross',relief=SUNKEN)
    ...
    def loadDir(self, dbg=False):
        ...
        self.outDir = os.path.join(r'./Labels', '%s' % (self.category.split("/")[-1]))
    ...
    def loadImage(self):
        ...
        newsize = (int(self.curimg_w / 1.5), int(self.curimg_h / 1.5))
        self.img = self.img.resize(newsize)

        self.curimg_w, self.curimg_h = self.img.size
        self.tkimg = ImageTk.PhotoImage(self.img)
        self.mainPanel.config(width=max(self.tkimg.width() - 100, 400), height=max(self.tkimg.height() - 100, 400))
        # self.mainPanel.config(width=max(self.tkimg.width(), 400), height=max(800, 400))
        # self.mainPanel.config(width=max(5000, 400), height=max(450, 400))
        # self.mainPanel.config(width=2080, height=800)
        # self.mainPanel.config(highlightthickness=0)
        ######################################################################################################
        # sbarV = Scrollbar(orient=VERTICAL)
        # sbarH = Scrollbar(orient=HORIZONTAL)
        # sbarV.config(command=self.mainPanel.yview)
        ...
        # # self.mainPanel.pack(side=LEFT, expand=YES, fill=BOTH)
        #
        # self.mainPanel.config(scrollregion=(0, 0, self.curimg_w, self.curimg_h))
        #####################################################################################
        # self.mainPanel.config(width=50, height=50)
        self.mainPanel.create_image(0, 0, image=self.tkimg, anchor=NW)
        # self.mainPanel.create_image(self.curimg_w, self.curimg_h, image=self.tkimg, anchor=NW)
    ...
    def saveImage(self):
        ...
        #         print 'Image No. %d saved' % (self.cur)
        #     else:
        #         for bbox, bboxcls, imgi_d_l in zip(self.bbox_list, self.bboxListCls, self.image_list_id):
        #             xmin, ymin, xmax, ymax = bbox
        #             # b = (float(xmin), float(xmax), float(ymin), float(ymax))
        #             bb = (float(xmin), float(ymin), float(xmax), float(ymax))
        #             # bb=b
