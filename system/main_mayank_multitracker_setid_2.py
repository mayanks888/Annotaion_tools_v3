...
class LabelTool():
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    # self.mainPanel.create_text(int(tmp[2]+tmp[0])/2, tmp[1]-15,text=yolo_data[-1])
    ...
    def mouseClick(self, event):
        if self.STATE['click'] == 0:
            ...
            # self.mainPanel.create_text(int(x1 + x2) / 2, y1 - 15, text=str(self.image_id))
            ##############################################
    ...
    def delBBox(self):
        ...
        # self.mainPanel.delete(self.image_list_id[idx])
