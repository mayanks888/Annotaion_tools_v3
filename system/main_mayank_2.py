        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    tmp = [int(float(loop)) for loop in yolo_data[1:]]
    ...
    def tracker_update(self):
        ...
        with open(cool) as f:
            for (i, line) in enumerate(f):
                ...
                ############################################333
                tmp = [int(float(loop)) for loop in yolo_data[1:]]
                initial_bbox = [tmp[0], tmp[1], tmp[2], tmp[3]]
                ###########################3333
                imagepath = self.imageList[0]
                ################################
                image = cv2.imread(imagepath)
                self.tracker.track('ball', image, initial_bbox)
                # initial_bbox = [tmp[0], tmp[1], tmp[2], tmp[3]]
        # for bbox in zip(self.bboxList, self.bboxListCls):
        #     # tmp = [int(float(loop)) for loop in yolo_data[1:]]
        #     tmp=bbox
        #     initial_bbox = [tmp[0], tmp[1], tmp[2], tmp[3]]
        #     ###########################3333
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    imageRGB = image[:, :, ::-1]
                    bbox = self.tracker.track('ball', imageRGB)
                    # [int(i) for i in bbox]
                    bbox = map(int, bbox)
                    # iou = self.get_iou(bbox, bbox_previous)
