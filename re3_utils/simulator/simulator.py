...
def make_paths(train=True):
    ...
    IMAGE_NAMES = [image_datadir + '/' + line.strip() for line in
                   open(base_path + '/labels/' + dataType + '/image_names.txt')]
...
def render_patch(bbox, background, trackedObjects, cropSize=CROP_SIZE, cropPad=CROP_PAD):
    ...
    for obj in trackedObjects:
        ...
        if (cropCoords[2] - cropCoords[0] < 1 or
            ...
            featherCrop[2] = int(
                min((fullBBox[2] - boxPos[0]) * 1.0 / boxPosXYWH[2], 1) * FEATHER_WEIGHT_ARRAY.shape[1])
            featherCrop[3] = int(
                min((fullBBox[3] - boxPos[1]) * 1.0 / boxPosXYWH[3], 1) * FEATHER_WEIGHT_ARRAY.shape[0])
