...
class TFQueue(object):
    ...
    def __init__(self, sess, placeholders, max_queue_size, max_queue_uses, use_random_order, batch_size):
        ...
        with tf.device('/cpu:0'):
            self.queue = tf.FIFOQueue(self.max_queue_size,
                                      ...
                                      shapes=[placeholder.get_shape().as_list()[1:] for placeholder in
                                              self.placeholders])
            self.enqueue_op = self.queue.enqueue_many(self.placeholders)
            self.placeholder_outs = {self.placeholders[ii]: val for ii, val in
                                     enumerate(self.queue.dequeue_many(self.batch_size))}
    ...
    def get_feed_dict(self):
        ...
        if self.use_random_order:
            ...
            chosen_inds = np.lexsort(
                (np.random.random(len(self.data_buffer)), self.data_counts[:len(self.data_buffer)]))[
    ...
    def tf_enqueue_data(self):
        while True:
            if self.max_queue_uses > 0:
                ...
                while len(data_counts_curr_length[
                              data_counts_curr_length < self.max_queue_uses]) < self.enqueue_batch_size:
                    time.sleep(1)
                    data_counts_curr_length = self.data_counts[:len(self.data_buffer)]
            else:
